...
export function useStoreProducts(storeId?: string) {
  ...
  const publishProducts = useMutation({
    mutationFn: async (items: { product_id: string; online_price?: number; online_description?: string; is_featured?: boolean; is_active?: boolean; force_out_of_stock?: boolean; out_of_stock_threshold?: number }[]) => {
