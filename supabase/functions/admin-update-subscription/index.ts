...
serve(async (req) => {
  ...
  try {
    ...
    const { subscriptionId, subscribed, subscriptionEnd, amount } = await req.json();

    if (!subscriptionId) {
      throw new Error('subscriptionId est requis');
    }
    ...
    console.log(`Abonnement ${subscriptionId} mis à jour par l'admin ${user.id}`);

    return new Response(
      JSON.stringify({ success: true, message: 'Abonnement mis à jour avec succès' }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 }
    );

  } catch (error) {
    console.error('Erreur dans admin-update-subscription:', error);
    return new Response(
