    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {store?.is_published && (
        <div className="bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 rounded-xl p-4 flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2"><Badge className="bg-green-600 text-white">En ligne</Badge><span className="text-sm font-medium truncate">{storeUrl}</span></div>
      ...
      <div className="flex items-center justify-center gap-0 py-4">
        {STEPS.map((step, i) => (
          <div key={step.id} className="flex items-center">
            ...
            {i < STEPS.length - 1 && (
              <div className={`w-6 sm:w-12 h-0.5 mx-1 ${currentStep > step.id ? 'bg-primary' : 'bg-border'}`} />
            )}
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
            ...
            <>
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div><Label>Nom de la boutique *</Label><Input value={form.name} onChange={e => handleNameChange(e.target.value)} placeholder="Ma Boutique" /></div>
                  <div><Label>Description</Label><Textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} rows={3} placeholder="Décrivez votre boutique en quelques mots..." /></div>
                </CardContent>
              </Card>
              <Card>
                ...
                <CardContent className="space-y-3">
                  <div className="p-4 bg-muted rounded-xl space-y-2">
                    <div className="flex items-center gap-2 text-sm"><Link2 className="h-4 w-4 text-muted-foreground" /><span className="text-muted-foreground">stocknix.com/boutique/</span></div>
          ...
          {currentStep === 3 && (
            <>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div><Label>Frais de livraison (FCFA)</Label><Input type="number" min="0" value={form.delivery_fee} onChange={e => setForm(p => ({ ...p, delivery_fee: Number(e.target.value) }))} /></div>
                  <div><Label>Informations livraison</Label><Textarea value={form.delivery_info} onChange={e => setForm(p => ({ ...p, delivery_info: e.target.value }))} rows={2} /></div>
                  <div><Label>Livraison gratuite à partir de (FCFA)</Label><Input type="number" min="0" value={form.free_delivery_minimum} onChange={e => setForm(p => ({ ...p, free_delivery_minimum: Number(e.target.value) }))} /></div>
                </CardContent>
              </Card>
            </>
          )}
          ...
          {currentStep === 4 && (
            <Card>
              <CardHeader><CardTitle>⚙️ Options de la boutique</CardTitle></CardHeader>
              <CardContent className="space-y-5">
                {[
                  { key: 'show_stock', label: 'Afficher le stock', desc: 'Les clients voient "X en stock"' },
                  { key: 'allow_orders', label: 'Accepter les commandes', desc: 'Désactiver = boutique en lecture seule' },
                  { key: 'enable_reviews', label: 'Avis clients', desc: 'Permettre les avis et notes' },
                  { key: 'maintenance_mode', label: 'Mode maintenance', desc: 'Affiche "Bientôt disponible" aux visiteurs' },
                ].map(item => (
          ...
          <div className="flex justify-between">
            ...
            {currentStep < 4 ? (
              <Button onClick={() => setCurrentStep(s => Math.min(4, s + 1))} className="gap-2">
                Suivant <ChevronRight className="h-4 w-4" />
              </Button>
            ) : (
              <Button onClick={handleSave} disabled={upsertStore.isPending} className="gap-2">
                <Save className="h-4 w-4" /> {upsertStore.isPending ? "Enregistrement..." : "Enregistrer la boutique"}
              </Button>
            )}
          </div>
        </div>
