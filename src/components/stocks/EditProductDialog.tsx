...
export function EditProductDialog({ product, open, onOpenChange }: EditProductDialogProps) {
  ...
  React.useEffect(() => {
    if (product) {
      setFormData({ name: product.name || "", description: product.description || "", price: product.price || 0, quantity: product.quantity || 0, min_quantity: product.min_quantity || 10, category: product.category || "", sku: product.sku || "", icon_emoji: product.icon_emoji || "📦", icon_bg_color: product.icon_bg_color || "bg-blue", image_url: product.image_url || null, unit: "pièce" });
      ...
      <DialogContent className="max-w-sm sm:max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-lg flex items-center gap-3">
            ...
            {step === 1 ? "Identité" : step === 2 ? "Prix & Stock" : "Détails & Média"}
          </DialogTitle>
        </DialogHeader>
        <StepIndicator />
        <div className="space-y-3">
          ...
          {step === 3 && (<>
            ...
            <div><Label>Photo</Label><ImageCropUpload currentImageUrl={formData.image_url} onImageUploaded={url => setFormData(p => ({ ...p, image_url: url }))} /></div>
          </>)}
        </div>
