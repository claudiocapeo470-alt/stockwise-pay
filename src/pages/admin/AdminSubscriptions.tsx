...
import { useAdminSubscriptions, useAdminPaymentHistory, useExtendSubscription } from "@/hooks/useAdmin";
...
export default function AdminSubscriptions() {
  ...
  const { data: history = [] } = useAdminPaymentHistory(historyUserId);
  ...
  const stats = {
    active: subscriptions.filter((s: any) => getStatus(s) === 'active').length,
    trial: subscriptions.filter((s: any) => getStatus(s) === 'trial').length,
    expired: subscriptions.filter((s: any) => getStatus(s) === 'expired').length,
    revenue: subscriptions.filter((s: any) => getStatus(s) === 'active').reduce((sum: number, s: any) => sum + (s.plan_price || s.amount || 0), 0),
  };
  ...
  return (
    <div className="space-y-6">
                ...
                <TableRow key={sub.id}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1">
                      ...
                      <Button size="sm" variant="ghost" onClick={() => extendMutation.mutate({ subId: sub.id, days: 30, currentEnd: sub.subscription_end })} title="+30 jours">+30j</Button>
                      <Button size="sm" variant="ghost" onClick={() => setEditingSubscription(sub)}><Edit className="h-4 w-4" /></Button>
                    </div>
                  </TableCell>
                </TableRow>
              ))}
      ...
      {editingSubscription && (
        <EditSubscriptionDialog open={!!editingSubscription} onOpenChange={(o) => !o && setEditingSubscription(null)} subscription={editingSubscription} />
      )}
