...
export default function StoreConfig() {
  ...
  const handlePublish = async () => {
    try {
      ...
      toast.success(store?.is_published ? "Boutique dépubliée" : "🎉 Votre boutique est en ligne !");
    } catch (e: any) {
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Configurez et publiez votre boutique en ligne</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          ...
          {store && (
            <Button variant="outline" asChild className="gap-2"><a href={storeUrl} target="_blank" rel="noopener"><Eye className="h-4 w-4" />👁 Voir</a></Button>
          )}
          {store && (
            <Button onClick={handlePublish} disabled={togglePublish.isPending}
              className={`gap-2 ${store.is_published ? 'bg-warning hover:bg-warning/90 text-warning-foreground' : 'bg-green-600 hover:bg-green-700 text-white'}`}>
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              <div><Label>Nom de la boutique *</Label><Input value={form.name} onChange={e => handleNameChange(e.target.value)} placeholder="Ma Super Boutique" /></div>
              <div><Label>Description</Label><Textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} placeholder="Décrivez votre boutique..." rows={3} /></div>
              <div><Label>Couleur principale</Label><div className="flex items-center gap-3"><input type="color" value={form.primary_color} onChange={e => setForm(p => ({ ...p, primary_color: e.target.value }))} className="h-10 w-14 rounded border cursor-pointer" /><span className="text-sm text-muted-foreground">{form.primary_color}</span></div></div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader><CardTitle>Contact & Infos</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div><Label>WhatsApp</Label><Input value={form.whatsapp} onChange={e => setForm(p => ({ ...p, whatsapp: e.target.value }))} placeholder="+225 07 00 00 00" /></div>
              ...
              <div><Label>Adresse / Zone de livraison</Label><Textarea value={form.address} onChange={e => setForm(p => ({ ...p, address: e.target.value }))} rows={2} /></div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {[
                { key: 'show_stock', label: 'Afficher le stock disponible', desc: 'Les clients voient "X en stock"' },
                { key: 'allow_orders', label: 'Accepter les commandes en ligne', desc: 'Désactiver = boutique en lecture seule' },
                { key: 'enable_reviews', label: 'Activer les avis clients', desc: '' },
                { key: 'maintenance_mode', label: 'Mode maintenance', desc: 'Affiche "Bientôt disponible"' },
              ].map(item => (
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div><Label>Frais de livraison (FCFA)</Label><Input type="number" min="0" value={form.delivery_fee} onChange={e => setForm(p => ({ ...p, delivery_fee: Number(e.target.value) }))} placeholder="0 = gratuit" /></div>
              <div><Label>Informations de livraison</Label><Textarea value={form.delivery_info} onChange={e => setForm(p => ({ ...p, delivery_info: e.target.value }))} placeholder="Délai, zones couvertes..." rows={2} /></div>
              <div><Label>Livraison gratuite à partir de (FCFA)</Label><Input type="number" min="0" value={form.free_delivery_minimum} onChange={e => setForm(p => ({ ...p, free_delivery_minimum: Number(e.target.value) }))} placeholder="0 = pas de minimum" /></div>
            </CardContent>
          </Card>
        </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[1,2,3,4].map(i => (
                      <div key={i} className="bg-card rounded-lg p-3 border border-border text-center">
                        ...
                        <div className="h-2 bg-primary/30 rounded w-1/2 mx-auto mt-1" />
                      </div>
                    ))}
                  </div>
                </div>
                {!store?.is_published && (
                  <div className="text-center py-2 bg-warning/10 text-warning text-xs font-medium border-t">APERÇU — Non publié</div>
                )}
              </div>
            </CardContent>
          </Card>
