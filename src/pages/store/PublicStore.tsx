...
import {
  ...
  Home, Store, User, Grid2X2, MessageCircle, Moon, Sun, ChevronRight
} from "lucide-react";
...
export default function PublicStore() {
  ...
  if (orderSuccess) return (
    <div className="flex items-center justify-center min-h-screen bg-background p-4">
      <div className="text-center space-y-4 max-w-sm">
        ...
        <Button onClick={() => { setOrderSuccess(null); setOrderForm({ name: "", phone: "+225 ", email: "", address: "", notes: "", payment_method: "cash_on_delivery" }); }} style={{ background: color }} className="text-white">Retour à la boutique</Button>
      </div>
    </div>
  );
  ...
  const ProductImage = ({ product, size = "md" }: { product: ProductData; size?: "sm" | "md" | "lg" }) => {
    ...
    if (product.image_url) {
      return <img src={product.image_url} alt={product.name} className={`${sizeClass} object-cover rounded-lg`} />;
    }
    return (
      <div className={`${sizeClass} rounded-lg flex items-center justify-center`} style={getIconBgStyle(product.icon_bg_color)}>
  ...
  const ProductCard = ({ product }: { product: ProductData }) => (
    <div className="bg-card rounded-2xl border border-border overflow-hidden hover:shadow-lg transition-all group relative">
      ...
      <div className="p-3 space-y-1">
        ...
        {product.quantity === 0 ? (
          ...
          <Button size="sm" className="w-full mt-2 gap-1 text-white" style={{ background: color }} onClick={() => addToCart(product)}>
  ...
  const HomePage = () => (
    <div className="pb-20">
      {/* Hero */}
      <div className="relative h-64 sm:h-80 bg-gradient-to-br from-gray-300 to-gray-400 flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center text-white px-4 space-y-3">
          <h1 className="text-2xl sm:text-3xl font-bold">{store.description || `Bienvenue chez ${store.name}`}</h1>
          <p className="text-sm opacity-80">Votre boutique de confiance</p>
          <Button onClick={() => setActivePage("shop")} className="text-white gap-2 rounded-full px-6 h-12" style={{ background: color }}>
            <ShoppingCart className="h-4 w-4" /> Commencer mes achats <ChevronRight className="h-4 w-4" />
          </Button>
        </div>
      </div>
      ...
      {featuredProducts.length > 0 && (
        <div className="px-4 py-6">
          <h2 className="text-lg font-bold mb-4">⭐ Produits vedettes</h2>
      ...
      {categories.length > 0 && (
        <div className="px-4 py-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {categories.map(cat => (
              <button key={cat} onClick={() => { setActiveCategory(cat); setActivePage("shop"); }}
                className="p-4 rounded-2xl border border-border bg-card hover:shadow-md transition-all text-left">
                <p className="font-semibold text-sm">{cat}</p>
                <p className="text-xs text-muted-foreground">{products.filter(p => p.category === cat).length} produits</p>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="px-4 py-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold">🛍️ Nos produits</h2>
          <Button variant="ghost" size="sm" onClick={() => setActivePage("shop")} style={{ color }}>Voir tout</Button>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
    ...
    <div className="pb-20">
      ...
      <div className="overflow-x-auto border-b border-border">
        <div className="flex gap-2 p-3">
          <button onClick={() => setActiveCategory(null)} className={`px-3 py-1.5 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${!activeCategory ? 'text-white' : 'bg-muted text-muted-foreground'}`} style={!activeCategory ? { background: color } : {}}>Tous</button>
          {categories.map(cat => (
            <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-3 py-1.5 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${activeCategory === cat ? 'text-white' : 'bg-muted text-muted-foreground'}`} style={activeCategory === cat ? { background: color } : {}}>{cat}</button>
          ))}
        </div>
      </div>
  ...
  const AccountPage = () => (
    <div className="pb-20 px-4 py-6 space-y-4">
      ...
      <div className="space-y-3">
        <button onClick={() => setShowCart(true)} className="w-full flex items-center gap-3 p-4 rounded-xl border border-border bg-card hover:bg-muted transition-colors text-left">
          <ShoppingCart className="h-5 w-5 text-muted-foreground" />
          <div className="flex-1"><p className="font-medium">Mon panier</p><p className="text-xs text-muted-foreground">{totalItems} article(s)</p></div>
          <ChevronRight className="h-4 w-4 text-muted-foreground" />
        </button>
        <button onClick={() => { setActivePage("shop"); setActiveCategory(null); const favIds = [...favorites]; if (favIds.length === 0) return; }}
          ...
          <div className="flex-1"><p className="font-medium">Mes favoris</p><p className="text-xs text-muted-foreground">{favorites.size} produit(s)</p></div>
          <ChevronRight className="h-4 w-4 text-muted-foreground" />
        </button>
        {store.phone && (
          <a href={`tel:${store.phone}`} className="w-full flex items-center gap-3 p-4 rounded-xl border border-border bg-card hover:bg-muted transition-colors text-left">
            <MessageCircle className="h-5 w-5 text-muted-foreground" />
            <div className="flex-1"><p className="font-medium">Nous contacter</p><p className="text-xs text-muted-foreground">{store.phone}</p></div>
            <ChevronRight className="h-4 w-4 text-muted-foreground" />
          </a>
        )}
        {store.address && (
  ...
  // PAGE: Search
  const SearchPage = () => (
    <div className="pb-20 px-4 py-4 space-y-4">
      <div className="relative">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
        <Input placeholder="Rechercher un produit..." value={search} onChange={e => setSearch(e.target.value)} className="pl-10 h-12 text-base rounded-full" autoFocus />
      </div>
      {search ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {filtered.map(p => <ProductCard key={p.id} product={p} />)}
  ...
  const CategoriesPage = () => (
    <div className="pb-20 px-4 py-6 space-y-4">
      <h2 className="text-lg font-bold">📂 Toutes les catégories</h2>
      <div className="grid grid-cols-2 gap-3">
        <button onClick={() => { setActiveCategory(null); setActivePage("shop"); }}
          className="p-4 rounded-2xl border border-border bg-card hover:shadow-md transition-all text-left" style={{ borderColor: !activeCategory ? color : undefined }}>
          <p className="text-2xl mb-2">🛍️</p>
          <p className="font-semibold text-sm">Tous les produits</p>
    ...
    <div className={`min-h-screen bg-background ${darkMode ? 'dark' : ''}`}>
        ...
        <div className="flex items-center justify-between px-4 h-14">
          ...
          <div className="flex items-center gap-2">
            {store.logo_url ? (
              ...
              <div className="h-9 w-9 rounded-lg flex items-center justify-center text-white font-bold text-sm" style={{ background: color }}>
                {store.name.substring(0, 2).toUpperCase()}
              </div>
            )}
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => setDarkMode(!darkMode)} className="h-9 w-9 rounded-full border border-border flex items-center justify-center">
              {darkMode ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}
            </button>
            <button onClick={() => setShowCart(true)} className="relative h-9 w-9 flex items-center justify-center">
              <ShoppingCart className="h-5 w-5" />
              {totalItems > 0 && <span className="absolute -top-1 -right-1 h-5 w-5 rounded-full text-[10px] flex items-center justify-center text-white font-bold" style={{ background: color }}>{totalItems}</span>}
            </button>
          </div>
        </div>
      </header>
      ...
      {store.whatsapp && (
        <a href={`https://wa.me/${store.whatsapp.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer"
          className="fixed bottom-20 left-4 z-30 h-12 w-12 rounded-full bg-green-500 text-white flex items-center justify-center shadow-lg hover:scale-110 transition-transform">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-border bg-background">
        <div className="flex items-center justify-around h-16">
          {navItems.map(item => {
            ...
            return (
              <button key={item.page} onClick={() => { setActivePage(item.page); if (item.page !== "search") setSearch(""); }}
                className="flex flex-col items-center justify-center flex-1 h-full gap-1 transition-colors">
                <item.icon className={`h-5 w-5 ${active ? '' : 'text-muted-foreground'}`} style={active ? { color } : {}} strokeWidth={active ? 2.5 : 2} />
                <span className={`text-[10px] ${active ? 'font-semibold' : 'text-muted-foreground'}`} style={active ? { color } : {}}>{item.label}</span>
                {active && <div className="absolute top-0 h-0.5 w-8 rounded-full" style={{ background: color }} />}
              </button>
            );
          })}
        </div>
          ...
          <div className="absolute right-0 top-0 h-full w-full sm:w-96 bg-background border-l border-border flex flex-col animate-slide-in-right">
            ...
            {cart.length > 0 && (
              <div className="p-4 border-t border-border space-y-3">
                ...
                <Button className="w-full h-12 text-white font-bold rounded-full" style={{ background: color }} onClick={() => { setShowCart(false); setShowCheckout(true); }}>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-background rounded-2xl border border-border w-full max-w-md max-h-[90vh] overflow-y-auto p-6 space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div><label className="text-sm font-medium">Téléphone *</label><Input value={orderForm.phone} onChange={e => setOrderForm(p => ({ ...p, phone: e.target.value }))} placeholder="+225 07 00 00 00" required /></div>
              <div><label className="text-sm font-medium">Email</label><Input value={orderForm.email} onChange={e => setOrderForm(p => ({ ...p, email: e.target.value }))} type="email" /></div>
              ...
              <Button className="w-full h-12 text-white font-bold rounded-full" style={{ background: color }} onClick={submitOrder} disabled={!orderForm.name || !orderForm.phone}>
