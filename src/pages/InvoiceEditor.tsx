...
import { Plus, Trash2, Save, ArrowLeft, ArrowRight, Check, User, FileText, Package, CheckCircle2 } from "lucide-react";
...
const STEPS = [
  ...
  { label: "Résumé", icon: CheckCircle2 },
];
...
export default function InvoiceEditor({ documentType }: InvoiceEditorProps) {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-5xl mx-auto px-2 sm:px-4">
      ...
      {step === 0 && (
        <Card>
          <CardHeader><CardTitle className="text-base sm:text-lg flex items-center gap-2"><User className="h-4 w-4 text-primary" /> Informations Client</CardTitle></CardHeader>
      ...
      {step === 1 && (
        <Card>
          <CardHeader><CardTitle className="text-base sm:text-lg flex items-center gap-2"><FileText className="h-4 w-4 text-primary" /> Informations du Document</CardTitle></CardHeader>
          <CardContent className="space-y-3 sm:space-y-4">
            <div>
              <Label className="text-xs sm:text-sm">Statut <span className="text-muted-foreground font-normal">(facultatif)</span></Label>
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-base sm:text-lg flex items-center gap-2"><Package className="h-4 w-4 text-primary" /> Articles / Prestations</CardTitle>
        ...
        <div className="space-y-4">
          {!settings && (
            <Card className="border-yellow-500 bg-yellow-50 dark:bg-yellow-950/20">
              <CardContent className="pt-4"><p className="text-xs text-yellow-800 dark:text-yellow-200">Configurez vos informations d'entreprise dans les paramètres.</p></CardContent>
            </Card>
          )}
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="border rounded-lg overflow-hidden">
                <table className="w-full text-sm table-fixed sm:table-auto">
                  <thead className="bg-muted"><tr><th className="text-left p-2 w-auto">Description</th><th className="text-right p-2 w-20 whitespace-nowrap">Qté</th><th className="text-right p-2 w-32 whitespace-nowrap">Total</th></tr></thead>
                  <tbody>
                    {items.map((item, i) => (
